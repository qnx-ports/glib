...
basic (void)
{
  ...
  server = g_notification_server_new ("gtk");
...
test_properties (void)
{
  ...
  g_notification_add_button_with_purpose (n, "label1", "some.purpose", "app.action1::target1");
  g_notification_add_button_with_purpose_and_target (n, "label", "x-gnome.purpose", "app.action2", "s", "bla");
  ...
  g_assert (rn->display_hint == G_NOTIFICATION_DISPLAY_HINT_TRANSIENT);

  g_assert_cmpint (rn->buttons->len, ==, 3);
  ...
  g_assert_cmpstr (b->label, ==, "label1");
  g_assert_cmpstr (b->purpose, ==, "some.purpose");
  g_assert_cmpstr (b->action_name, ==, "app.action1");
  g_assert_cmpstr (g_variant_get_string (b->target, NULL), ==, "target1");

  b = (Button*)rn->buttons->pdata[2];
